const CACHE='fluidite-v2.10.3-photo-vault';
...
// Build V2.10.3 photo-vault anti-cache
self.addEventListener('install', event => { self.skipWaiting(); });
self.addEventListener('activate', event => {
  event.waitUntil((async()=>{
    const keys = await caches.keys();
    await Promise.all(keys.filter(k => k !== 'fluidite-v2.10.3-photo-vault').map(k => caches.delete(k)));
