const CACHE='fluidite-v2.10.4-suggestion-action';
...
// Build V2.10.4 suggestion-action anti-cache
self.addEventListener('install', event => { self.skipWaiting(); });
self.addEventListener('activate', event => {
  event.waitUntil((async()=>{
    const keys = await caches.keys();
    await Promise.all(keys.filter(k => k !== 'fluidite-v2.10.4-suggestion-action').map(k => caches.delete(k)));
