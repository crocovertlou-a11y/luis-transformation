...
function openSheet(kind){
  ...
  if(kind==='hydrationQuick') return showSheet(`<h2>Hydratation</h2><p class="subtle">Ajoute simplement ce que tu viens de boire.</p><form id="hydrationQuickForm">${dateField('date',todayKey())}<div class="field"><label>Eau (L)</label><input name="water" type="number" min="0.1" max="5" step="0.1" inputmode="decimal" value="0.5" required></div><button class="action" type="submit">Ajouter</button></form>`);
...
async function saveHydrationQuick(e){
  e.preventDefault();const f=new FormData(e.currentTarget),water=num(f.get('water'));
  if(!water||water<=0)return;
  await LTDB.put('food',{id:uid(),date:f.get('date')||todayKey(),dateTime:new Date().toISOString(),mealType:'hydration',description:'Eau',protein:0,calories:0,carbs:0,fat:0,water,classic:false,source:'hydration',createdAt:new Date().toISOString()});
  toast('Hydratation enregistrée');$('#sheet')?.close();render();
}
