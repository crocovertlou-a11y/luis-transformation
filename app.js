  ...
  <div class="card training-v2-feature">
    ...
    <div id="smartTrainingSuggestion">
      ...
      <div class="card-actions">
        <button class="action orange" id="startSuggestedTraining" type="button">${recovery?'Démarrer la récupération active':'Voir et démarrer la séance'}</button>
...
function bindSheet(){
  ...
  $('#checkinForm')?.addEventListener('submit',saveCheckin); $('#workoutForm')?.addEventListener('submit',saveWorkout); $('#cardioForm')?.addEventListener('submit',saveCardio); $('#cardioImportInput')?.addEventListener('change',handleCardioImport); $('#cardioImportConfirmForm')?.addEventListener('submit',saveImportedCardio); $('#fetchStravaActivities')?.addEventListener('click',fetchStravaActivities); $('#startSuggestedTraining')?.addEventListener('click',async()=>{const ws=(await LTDB.all('workouts')).sort((a,b)=>b.date.localeCompare(a.date));let decision=null;try{const [cs,fs,ks]=await Promise.all([LTDB.all('cardio'),LTDB.all('food'),LTDB.all('checkins')]);cs.sort((a,b)=>b.date.localeCompare(a.date));const today=todayKey(),check=ks.find(x=>x.date===today)||null,todayWorkout=ws.find(x=>x.date===today)||null,todayCardio=cs.filter(x=>x.date===today);decision=fluidityEngine(check,todayWorkout,todayCardio,ws,cs,fs,ks)}catch(err){console.warn('Unable to refresh training decision',err)}const w=fluidityWorkoutForDecision(decision,ws);await workoutDetailSheet(w)}); $('#askSmartTraining')?.addEventListener('click',loadSmartTrainingSuggestion); $('#stravaConfirmForm')?.addEventListener('submit',saveStravaCardio); $('#hydrationQuickForm')?.addEventListener('submit',saveHydrationQuick); $('#foodForm')?.addEventListener('submit',saveFood); $('#foodSearchForm')?.addEventListener('submit',searchFoods); $('#foodSearchConfirmForm')?.addEventListener('submit',saveSearchedFood); $('#barcodeForm')?.addEventListener('submit',lookupBarcode); $('#startBarcodeCamera')?.addEventListener('click',startBarcodeCamera); $('#toggleManualBarcode')?.addEventListener('click',()=>$('#barcodeForm')?.classList.toggle('hidden')); $('#barcodeConfirmForm')?.addEventListener('submit',saveBarcodeFood); $('#aiFoodConfirmForm')?.addEventListener('submit',saveAIFood);
