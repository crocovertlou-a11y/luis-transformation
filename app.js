...
async function renderEvolution(checkins,workouts,cardio){
  ...
  <div class="card photo-journal"><div class="card-kicker">Photos</div><div class="photo-title-row"><div><h3>Voir le changement</h3><p class="subtle">Même cadrage, même vue, une date. L’analyse IA viendra ensuite.</p></div><button class="action compact" data-open="progressPhoto">Ajouter</button></div>${gallery||'<div class="empty">Tes photos d’évolution apparaîtront ici en petites vignettes, classées par date.</div>'}${Object.keys(groups).length>=2?`<div class="photo-compare-launch"><button class="action secondary" data-open="photoCompare">Comparer deux dates</button></div>`:''}</div>
...
function openSheet(kind){
  ...
  if(kind==='photoCompare') {
    LTDB.all('photos').then(photos=>{
      ...
      if(dates.length<2){toast('Il faut au moins deux dates');return}
