...
async function renderToday(today,todayWorkout,todayCardio,protein,calories,foodCount){
  ...
  const checkin=`<section class="today-wellbeing clickable" data-open="checkin"><div class="today-card-head"><h2>♡ &nbsp;Comment vas-tu aujourd’hui ?</h2><span>${today?'Modifier ›':'Renseigner ›'}</span></div>${today?`<div class="today-vitals">${vitals}</div>`:`<div class="today-empty-feel">Quelques secondes suffisent pour adapter ta journée.</div>`}<button type="button" class="hydration-line" data-sheet="hydrationQuick"><div><span class="water-icon">💧</span><strong>Hydratation</strong> <b>${water.toFixed(1).replace('.',',')} L / ${waterTarget} L</b></div><div class="water-drops">${Array.from({length:6},(_,i)=>`<i class="${i<Math.round(water/waterTarget*6)?'on':''}">●</i>`).join('')} <span>›</span></div></button></section>`;
...
async function renderEvolution(checkins,workouts,cardio){
  ...
  <div class="card" style="margin-top:14px"><div class="card-kicker">Comprendre</div><h3>Pourquoi cette lecture ?</h3><p class="subtle">${sorted.length<2?'Je manque encore de mesures comparables pour conclure. Continue simplement à enregistrer poids, tour de taille et photos dans des conditions similaires.':`Je m’appuie sur ${sorted.length} relevé${sorted.length>1?'s':''}, l’évolution du poids${waistRows.length>1?' et du tour de taille':''}, ainsi que sur ${activities} activité${activities>1?'s':''} sur 30 jours. Je privilégie la tendance plutôt qu’une mesure isolée.`}</p><div class="card-actions"><button class="action secondary" data-open="details">Explorer les données</button></div></div>`;
}
...
async function renderProfile(){
  ...
  <div class="card"><div class="card-kicker">Ce que tu sais de moi</div><div class="list"><button type="button" class="list-row" data-sheet="goalEdit"><div><strong>Objectif principal</strong><div class="status">${escapeHtml(state.profile.goal||'À définir')}</div></div><span class="pill">Modifier ›</span></button><div class="list-row"><div><strong>Alimentation</strong><div class="status">${state.profile.nutritionEnabled?'Accompagnement actif':'Masquée'}</div></div><span class="pill">Choix</span></div></div></div>
...
function bindPage(){
  ...
  document.querySelectorAll('[data-fluidity-force]').forEach(b=>b.addEventListener('click',async()=>{const workouts=(await LTDB.all('workouts')).sort((a,b)=>b.date.localeCompare(a.date));const w=suggestWorkout(workouts);if(b.dataset.fluidityForce==='detail')return workoutDetailSheet(w);await workoutDetailSheet(w);chosenWorkoutForm();}));
...
function showSheet(html,parent=null){ stopBarcodeCamera(); stopProgressCamera(); sheetParent=parent; $('#sheetContent').innerHTML=`<button class="sheet-x" type="button" data-close aria-label="Fermer">×</button>${html}`; $('#sheet').showModal(); bindSheet(); updateAllRanges(); }
function closeCurrentSheet(){ stopBarcodeCamera();stopProgressCamera();const parent=sheetParent;sheetParent=null;if(parent==='nutritionHub')return nutritionHubSheet();$('#sheet').close(); }
...
function openSheet(kind){
  ...
  if(kind==='goalEdit') return showSheet(`<h2>Objectif principal</h2><p class="subtle">C’est ton objectif. Fluidité peut t’aider à ajuster le chemin, mais ne le change jamais sans toi.</p><form id="goalForm"><div class="field"><label>Objectif</label><input name="goal" value="${escapeHtml(state.profile.goal||'')}" placeholder="Ex. Recomposition corporelle" required></div><button class="action" type="submit">Enregistrer</button></form>`);
  if(kind==='checkin') {
...
function bindSheet(){
  ...
  document.querySelectorAll('[data-close]').forEach(b=>b.addEventListener('click',closeCurrentSheet));
...
async function saveGoal(e){e.preventDefault();const f=new FormData(e.currentTarget),goal=String(f.get('goal')||'').trim();if(!goal)return;state.profile.goal=goal;await LTDB.put('profile',state.profile);$('#sheet').close();toast('Objectif mis à jour');render();}
