...
function formatSleepDuration(value,minutes=null){
  const total=minutes!=null?Math.round(Number(minutes)):Math.round((Number(value)||0)*60);
...
function fluidityEngine(today,todayWorkout,todayCardio,workouts,cardio,food,checkins){
  ...
  const garminReadiness=today?.garminHealth?.readiness?.status||'insufficient';
  if(todayWorkout){
  ...
  if(garminReadiness==='recover' && energy<=3){
    return {decision:'recovery',allowedActions:['adapted_session','recovery'],confidence:'medium',priority:'high',shouldSpeak:true,title:'Les signaux invitent à récupérer',message:'Plusieurs signaux Garmin confirmés s’écartent de ta tendance personnelle. Je privilégie une récupération active ou une séance très légère, à confirmer avec ton ressenti.',action:'training',healthContext:today.garminHealth};
  }
  if(garminReadiness==='watch' && (energy<=3||Number(today?.recovery||3)<=3)){
    return {decision:'adapted_session',allowedActions:['planned_session','adapted_session','recovery'],confidence:'medium',priority:'high',shouldSpeak:true,title:'Une séance prudente aujourd’hui',message:'Plusieurs signaux Garmin confirmés méritent attention. Ton ressenti reste décisif : si tu t’entraînes, garde une intensité maîtrisée.',action:'training',healthContext:today.garminHealth};
  }
...
async function renderToday(today,todayWorkout,todayCardio,protein,calories,foodCount){
  ...
  const vitals=today?`${metric('⚡','Énergie',`${today.energy??'—'}/5`,Number(today.energy||0)*20,'orange')}${metric('◉','Stress',`${today.stress??'—'}/5`,Number(today.stress||0)*20,'purple')}${metric('☾','Sommeil',today.sleep!=null?formatSleepDuration(today.sleep,today.sleepMinutes):'—',Math.min(100,Number(today.sleep||0)/8*100),'blue')}${metric('♡','Récupération',recovery!=null?`${recovery}/5`:'—',recovery!=null?recovery*20:50,'green')}${metric('◔','Faim',`${today.hunger??'—'}/5`,Number(today.hunger||0)*20,'orange')}`:'';
...
function renderCheckinSummary(t){
  const rows=[];
  if(t.sleep!=null) rows.push(`<div><span>Sommeil</span><strong>${formatSleepDuration(t.sleep,t.sleepMinutes)}</strong></div>`);
  ...
  <div class="training-history-grid">
    ...
    <div class="card compact-history-card">
      ...
      <div class="history-card-actions">
        <button class="action compact full" data-open="cardio">Ajouter / compléter manuellement</button>
        <button class="action secondary compact full" data-garmin-import>Importer la capsule Garmin</button>
        <button class="action secondary compact full" data-open="cardioImport">Importer un fichier GPX / TCX / FIT</button>
...
async function companionSnapshot(){
  ...
  const context={
    ...
    todayCardio:todayCardio.map(c=>({type:c.type||null,name:c.name||null,distance:c.distance??null,duration:c.durationLabel||null,durationSeconds:c.durationSeconds??null,heartRateAvg:c.heartRateAvg??null,cadenceAvg:c.cadenceAvg??c.cadence??null,calories:c.calories??null,source:c.importSource||c.source||null})),
    ...
    recentCardio:recentCardio.slice(0,8).map(c=>({date:c.date,type:c.type||null,name:c.name||null,distance:c.distance??null,duration:c.durationLabel||null,durationSeconds:c.durationSeconds??null,heartRateAvg:c.heartRateAvg??null,cadenceAvg:c.cadenceAvg??c.cadence??null,source:c.importSource||c.source||null})),
...
function updateRange(r){ const out=document.querySelector(`[data-output="${r.name}"]`); if(out) out.value=r.name==='sleep'?formatSleepDuration(r.value):`${r.value}${r.dataset.rangeUnit||''}`; }
async function saveCheckin(e){e.preventDefault(); const f=new FormData(e.currentTarget); const date=f.get('date')||todayKey(); const previous=await LTDB.get('checkins',date)||{}; const sleep=num(f.get('sleep')); const row={...previous,id:date,date,sleep,sleepMinutes:sleep==null?null:Math.round(sleep*60),sleepSource:'manual',energy:num(f.get('energy')),stress:num(f.get('stress')),recovery:num(f.get('recovery')),hunger:num(f.get('hunger')),weight:num(f.get('weight')),waist:num(f.get('waist')),source:'manual',updatedAt:new Date().toISOString()}; await LTDB.put('checkins',row); $('#sheet').close(); toast('Point du jour enregistré'); render();}
