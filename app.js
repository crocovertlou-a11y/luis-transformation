...
function renderToday(today,todayWorkout,protein){
  ...
  return `
    ...
    <div class="card clickable" data-open="checkin"><div class="card-kicker">Ressenti</div><h3>${today?'Ton point du jour':'Comment vas-tu aujourd’hui ?'}</h3>${today?renderCheckinSummary(today):'<p class="subtle">Un geste par donnée. Quelques secondes suffisent.</p>'}</div>
    <div class="card clickable" data-route-card="training"><div class="card-kicker">Force</div><h3>${todayWorkout?escapeHtml(todayWorkout.name||'Séance enregistrée'):'Ta prochaine séance peut être préparée ici'}</h3><p class="subtle">Suggestion de séance + journal de tes exercices, sans suivi live imposé.</p><div class="card-actions"><button class="action secondary">Voir Force</button></div></div>
    ${state.profile.nutritionEnabled?`<div class="card clickable" data-open="nutritionHub"><div class="card-kicker">Alimentation</div><h3>${protein?`${Math.round(protein)} / ${proteinTarget} g de protéines`:'Accompagnement actif'}</h3><p class="subtle">Repas, macros utiles, idées et classiques — seulement si tu veux les utiliser.</p><div class="card-actions"><button class="action secondary">Ouvrir Alimentation</button></div></div>`:''}
  `;
}
...
async function renderTraining(){
  ...
  return `<section class="hero"><div class="hello">Entraînement</div><div class="subtle">Des propositions utiles, puis ton journal. Pas de tracker live à nourrir.</div></section>
  <div class="card training-feature"><div class="card-kicker">Force · séance suggérée</div><h3>${suggestion.title}</h3><p class="subtle">${suggestion.subtitle}</p><div class="exercise-chips">${suggestion.exercises.map(x=>`<span>${escapeHtml(x)}</span>`).join('')}</div><div class="card-actions"><button class="action" data-open="workout">Enregistrer cette séance</button><button class="action secondary" data-open="workoutIdeas">Autre suggestion</button></div></div>
  <div class="card"><div class="card-kicker">Journal Force</div><h3>Où tu en es, exercice par exercice</h3><div class="list exercise-log">${exerciseLog||'<div class="empty">Ton journal se construira à mesure que tu enregistres tes séances.</div>'}</div></div>
  <div class="card"><div class="card-kicker">Cardio</div><h3>${cardio.length?`${cardio.length} activité${cardio.length>1?'s':''} enregistrée${cardio.length>1?'s':''}`:'Course · vélo · natation · marche'}</h3><p class="subtle">Durée au format 40:00, distance, cadence, FC, dénivelé et calories si disponibles.</p><div class="card-actions"><button class="action" data-open="cardio">Ajouter une activité</button></div></div>
...
function suggestWorkout(workouts){
  const recent=workouts.slice(0,3).map(x=>(x.name||'').toLowerCase()).join(' ');
  if(/jamb|leg/.test(recent)) return {title:'Haut du corps · 40 min',subtitle:'Une proposition simple pour garder le rythme.',exercises:['Développé couché','Tractions','Rowing','Épaules','Abdos']};
  return {title:'Full body · 40 min',subtitle:'Une base équilibrée, facilement modifiable.',exercises:['Squat','Développé couché','Rowing','Élévations latérales','Gainage']};
}
...
function openSheet(kind){
  ...
  if(kind==='workout') return showSheet(`<h2>Enregistrer une séance Force</h2><form id="workoutForm"><div class="field"><label>Séance</label><input name="name" value="Full body" placeholder="Haut du corps"></div><div class="field"><label>Exercices / performance</label><textarea name="exercises" rows="6" placeholder="Développé couché | 80 kg × 8\nTractions | 8 reps\nRowing | 70 kg × 10"></textarea><div class="status">Une ligne par exercice. Le journal reprendra automatiquement ta dernière performance.</div></div><div class="field"><label>Durée</label><input name="duration" placeholder="40:00" inputmode="numeric"></div>${slider('effort','Ressenti','1','5','1','3','/5')}<div class="field"><label>Note</label><textarea name="notes" rows="2"></textarea></div><button class="action" type="submit">Enregistrer</button></form>`);
  ...
  if(kind==='photoFood') return showSheet(`<h2>Photo produit</h2><p class="subtle">Sélectionne une photo : elle sera prévisualisée avant toute sauvegarde.</p><input id="foodPhotoInput" type="file" accept="image/*"><div id="foodPhotoPreview" class="photo-preview empty">Aucune photo sélectionnée.</div>`);
...
async function mealIdeaSheet(){
  ...
  const suggestion=remain>45?'Poulet citron, pommes de terre et courgettes':remain>25?'Omelette méditerranéenne et salade':'Saumon, légumes et riz';
  return showSheet(`<h2>Une idée pour ce soir</h2><div class="recipe-card"><div class="card-kicker">Suggestion</div><h3>${suggestion}</h3><p>Il te reste environ <strong>${Math.round(remain)} g</strong> de protéines par rapport à ton repère actuel.</p>${classics.length?`<p class="status">Je garde aussi en tête tes classiques : ${classics.map(escapeHtml).join(', ')}.</p>`:''}<div class="card-actions"><button class="action" data-close>Ça me tente</button><button class="action secondary" data-sheet="mealIdea">Autre idée</button></div></div>`);
}
function bindSheet(){
