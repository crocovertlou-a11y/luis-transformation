...
function openSheet(kind){
  ...
  if(kind==='checkin') {
    ...
    LTDB.get('checkins',date).then(existing=>{
      const v=existing||{};
      const sleep=v.sleep??7, energy=v.energy??3, stress=v.stress??2, hunger=v.hunger??3;
      const weight=v.weight??'', waist=v.waist??'';
      showSheet(`<h2>Comment vas-tu aujourd’hui ?</h2><form id="checkinForm">${dateField('date',date)}${slider('sleep','Sommeil','0','12','0.25',String(sleep),' h')}${slider('energy','Énergie','1','5','1',String(energy),'/5')}${slider('stress','Stress','1','5','1',String(stress),'/5')}${slider('hunger','Faim','1','5','1',String(hunger),'/5')}<div class="field"><label>Poids (kg)</label><input name="weight" type="number" min="20" max="300" step="0.1" inputmode="decimal" placeholder="80.4" value="${weight}"></div><div class="field"><label>Tour de taille (cm)</label><input name="waist" type="number" min="30" max="250" step="0.1" inputmode="decimal" placeholder="90.0" value="${waist}"></div><button class="action" type="submit">${existing?'Mettre à jour':'Enregistrer'}</button></form>`);
    }).catch(err=>{console.error(err);toast('Impossible de charger le ressenti');});
    return;
  }
...
async function saveCheckin(e){e.preventDefault(); const f=new FormData(e.currentTarget); const date=f.get('date')||todayKey(); const existing=await LTDB.get('checkins',date); const weightRaw=f.get('weight'),waistRaw=f.get('waist'); const row={id:date,date,sleep:num(f.get('sleep')),energy:num(f.get('energy')),stress:num(f.get('stress')),hunger:num(f.get('hunger')),weight:weightRaw===''?(existing?.weight??null):num(weightRaw),waist:waistRaw===''?(existing?.waist??null):num(waistRaw),source:'manual',updatedAt:new Date().toISOString()}; await LTDB.put('checkins',row); $('#sheet').close(); toast(existing?'Ressenti mis à jour':'Point du jour enregistré'); render();}
...
function parseGPX(text){
 ...
 return{type:cardioTypeFromText(xmlVal(doc,'type')||xmlVal(doc,'name')),date:importCardioDate(times[0]),distance:dist/1000,durationSeconds:times.length>1?(times.at(-1)-times[0])/1000:0,elevationGain:up||null,heartRateAvg:hrn?hr/hrn:null,cadenceAvg:cadn?cad/cadn:null,calories:null,importSource:'GPX'};
}
function parseTCX(text){
 ...
 return{type:cardioTypeFromText(act.getAttribute('Sport')),date:importCardioDate(xmlVal(act,'Id')),distance:dist/1000,durationSeconds:sec,elevationGain:up||null,heartRateAvg:hrw?hr/hrw:null,cadenceAvg:cadw?cad/cadw:null,calories:cal||null,importSource:'TCX'};
}
...
function fitSession(v,o){const sports={1:'Course',2:'Vélo',5:'Natation',11:'Marche'},epoch=Date.UTC(1989,11,31)/1000;if(v[253]!=null)o.date=importCardioDate(new Date((v[253]+epoch)*1000));if(v[5]!=null)o.type=sports[v[5]]||o.type;if(v[7]!=null)o.durationSeconds=v[7]/1000;if(v[9]!=null)o.distance=v[9]/100000;if(v[11]!=null)o.calories=v[11];if(v[16]!=null)o.heartRateAvg=v[16];if(v[18]!=null)o.cadenceAvg=v[18];if(v[22]!=null)o.elevationGain=v[22];return o}
