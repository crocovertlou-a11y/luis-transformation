...
function normalizeRunningCadence(value,type){const n=Number(value);if(!Number.isFinite(n)||n<=0)return null;return /course|run/i.test(String(type||''))&&n<120?n*2:n;}
...
function openSheet(kind){
  ...
  if(kind==='checkin') return LTDB.get('checkins',todayKey()).then(x=>{x=x||{};showSheet(`<h2>Comment vas-tu aujourd’hui ?</h2><form id="checkinForm">${dateField('date',x.date||todayKey())}${slider('sleep','Sommeil','0','12','0.25',x.sleep??7,' h')}${slider('energy','Énergie','1','5','1',x.energy??3,'/5')}${slider('stress','Stress','1','5','1',x.stress??2,'/5')}${slider('hunger','Faim','1','5','1',x.hunger??3,'/5')}<div class="field"><label>Poids (kg)</label><input name="weight" type="number" min="20" max="300" step="0.1" inputmode="decimal" value="${x.weight??''}" placeholder="80.4"></div><div class="field"><label>Tour de taille (cm)</label><input name="waist" type="number" min="30" max="250" step="0.1" inputmode="decimal" value="${x.waist??''}" placeholder="90.0"></div><button class="action" type="submit">${x.id?'Mettre à jour':'Enregistrer'}</button></form>`)});
  if(kind==='profileGoal') return showSheet(`<h2>Modifier mon objectif</h2><form id="profileGoalForm"><div class="field"><label>Objectif actuel</label><input name="goal" value="${escapeHtml(state.profile.goal||'')}" placeholder="Ex. Voir mes abdos"></div><button class="action" type="submit">Enregistrer l’objectif</button></form>`);
  if(kind==='profileVision') return showSheet(`<h2>Modifier ma vision</h2><form id="profileVisionForm"><div class="field"><label>Ma vision</label><textarea name="vision" rows="4" placeholder="Ex. Un physique sec, athlétique et durable.">${escapeHtml(state.profile.vision||'')}</textarea></div><button class="action" type="submit">Enregistrer la vision</button></form>`);
...
async function updateActivity(e){
  ...
  if(kind==='Cardio'){
    ...
    await LTDB.put(store,{...old,date,type:f.get('type'),distance:num(f.get('distance')),durationSeconds:seconds,durationLabel:label,heartRateAvg:num(f.get('hr')),cadenceAvg:normalizeRunningCadence(num(f.get('cadence')),f.get('type')),elevationGain:num(f.get('elevation')),calories:num(f.get('calories')),updatedAt:new Date().toISOString()});
  } else {
...
async function saveProfileGoal(e){e.preventDefault();const f=new FormData(e.currentTarget);state.profile.goal=String(f.get('goal')||'').trim()||'À définir';state.profile.updatedAt=new Date().toISOString();await LTDB.put('profile',state.profile);$('#sheet').close();toast('Objectif mis à jour');render();}
async function saveProfileVision(e){e.preventDefault();const f=new FormData(e.currentTarget);state.profile.vision=String(f.get('vision')||'').trim();state.profile.updatedAt=new Date().toISOString();await LTDB.put('profile',state.profile);$('#sheet').close();toast('Vision mise à jour');render();}
...
function parseGPX(text){
 ...
 return{type:cardioTypeFromText(xmlVal(doc,'type')||xmlVal(doc,'name')),date:importCardioDate(times[0]),distance:dist/1000,durationSeconds:times.length>1?(times.at(-1)-times[0])/1000:0,elevationGain:up||null,heartRateAvg:hrn?hr/hrn:null,cadenceAvg:normalizeRunningCadence(cadn?cad/cadn:null,cardioTypeFromText(xmlVal(doc,'type')||xmlVal(doc,'name'))),calories:null,importSource:'GPX'};
}
function parseTCX(text){
 ...
 return{type:cardioTypeFromText(act.getAttribute('Sport')),date:importCardioDate(xmlVal(act,'Id')),distance:dist/1000,durationSeconds:sec,elevationGain:up||null,heartRateAvg:hrw?hr/hrw:null,cadenceAvg:normalizeRunningCadence(cadw?cad/cadw:null,cardioTypeFromText(act.getAttribute('Sport'))),calories:cal||null,importSource:'TCX'};
}
...
function fitSession(v,o){const sports={1:'Course',2:'Vélo',5:'Natation',11:'Marche'},epoch=Date.UTC(1989,11,31)/1000;if(v[253]!=null)o.date=importCardioDate(new Date((v[253]+epoch)*1000));if(v[5]!=null)o.type=sports[v[5]]||o.type;if(v[7]!=null)o.durationSeconds=v[7]/1000;if(v[9]!=null)o.distance=v[9]/100000;if(v[11]!=null)o.calories=v[11];if(v[16]!=null)o.heartRateAvg=v[16];if(v[18]!=null)o.cadenceAvg=normalizeRunningCadence(v[18],o.type);if(v[22]!=null)o.elevationGain=v[22];return o}
...
async function saveSearchedFood(e){
 ...
 toast('Aliment enregistré');await nutritionHubSheet();render();
}
